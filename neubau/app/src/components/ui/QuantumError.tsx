...
export function QuantumError({ type = '404', reset }: QuantumErrorProps) {
    ...
    const title = type === '404' ? '404' : 'FEHLER';
    const subtitle = type === '404' ? 'SEITE NICHT GEFUNDEN' : 'FEHLER_ABGEFANGEN';
    const description = type === '404'
        ? 'Die gesuchte Seite ist nicht vorhanden.'
        : 'Ein interner technischer Fehler ist aufgetreten.';
    ...
    return (
        <div className="fixed inset-0 z-[99999] bg-white dark:bg-[#000000] flex flex-col items-center justify-center font-sans overflow-hidden select-none">
            ...
            >
                ...
                >
                    <Link href="/" className="group relative flex items-center gap-3 px-8 py-4 rounded-full bg-foreground text-background font-bold text-sm tracking-wide transition-all hover:pr-10">
                        <Home size={18} />
