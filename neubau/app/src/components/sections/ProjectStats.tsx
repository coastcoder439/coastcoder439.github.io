...
const calculateMetrics = () => {
    ...
    const visualPreviews = portfolioData.projects?.filter((project) => Boolean(project.image)).length || 0;
    const liveLinks = portfolioData.projects?.filter((project) => Boolean(project.demoUrl)).length || 0;
    const repoLinks = portfolioData.projects?.filter((project) => Boolean(project.repoUrl)).length || 0;
...
export function ProjectStats({ isLowPowerMode }: { isLowPowerMode?: boolean }) {
    ...
    const stats = [
        ...
        {
            value: String(metrics.liveLinks),
            label: 'Live-Vorschauen',
            ...
            <div className="container-creative relative z-10 px-4 sm:px-6 md:px-8">
                ...
                >
                    ...
                    <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black text-foreground mb-3">
                        Echt gebaut. Echt verlinkt.
                    </h2>

                    <p className="text-base sm:text-lg text-muted-foreground max-w-2xl mx-auto">
                        Jede Kennzahl stammt direkt aus den acht Portfolioeinträgen – ohne hochgerechnete Werte.
                    </p>
                </motion.div>
