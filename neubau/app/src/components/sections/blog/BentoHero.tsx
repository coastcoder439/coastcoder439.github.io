...
import {
  ...
  Code, TerminalWindow, ShieldCheck, Robot, Atom, Database, Cpu, FileText, Lightning, Globe, Stack
...
const getIconForText = (text: string) => {
  ...
  if (t.includes('ai') || t.includes('agent') || t.includes('intelligence') || t.includes('machine learning')) return Robot;
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <div className="flex items-center gap-6">
            ...
            >
              Notiz öffnen <ArrowUpRight className="w-3 h-3" />
            </Link>
...
function StatsVisual({ categoryStats }: { categoryStats: { name: string, count: number }[] }) {
  const totalArticles = categoryStats.reduce((sum, category) => sum + category.count, 0);
  const maxCount = Math.max(1, ...categoryStats.map((category) => category.count));
  const bars = categoryStats.map((category) => Math.max(20, Math.round((category.count / maxCount) * 95)));
  const labels = categoryStats.map((category) => category.name.slice(0, 4).toUpperCase());
  ...
  return (
    <div className="w-full h-full flex flex-col gap-3.5 justify-between p-2">
      <div className="flex gap-4 pt-[0.625rem] pr-[0.625rem] pb-0.5 pl-0.5">
        {[
          { label: "Notizen", value: String(totalArticles), trend: "belegt" },
          { label: "Kategorien", value: String(categoryStats.length), trend: "gesamt" },
        ].map((s, i) => {
              ...
              >
                <div className="flex flex-col min-w-0">
                  ...
                  <div className="flex items-center gap-1.5 mt-2">
                    ...
                    <span className="text-[8px] text-muted-foreground/50 font-mono">aktuell</span>
                  </div>
                </div>
...
function RecentPublicationsVisual({ blogs }: { blogs: any[] }) {
  ...
  const logs = blogs.slice(0, 7).map((b, i) => ({
    ...
    t: new Date(b.date).toLocaleDateString('de-DE', { month: 'short', day: 'numeric' })
  }));
      ...
      <AnimatePresence mode="popLayout">
        {topProjects.map((p, i) => {
          ...
          return (
            ...
            <span className="text-[10px] text-muted-foreground mt-1.5 font-mono uppercase tracking-widest">{new Date(p.startDate).getFullYear()} • {p.status === 'completed' ? 'Abgeschlossen' : 'In Arbeit'}</span>
                  </div>
                </div>
...
export const BentoHero = ({ isLowPowerMode }: { isLowPowerMode?: boolean }) => {
  ...
  const CARDS = [
    {
      title: "Ausgewählte Produktnotiz",
    description: "Technische Entscheidungen und Erkenntnisse aus echten Produktständen.",
    ...
    {
      title: "Die Galerie",
      description: "Belegte Ansichten aus Repositories und dokumentierten Abnahmen.",
      visual: (
    ...
    {
      title: "Notiz-Statistik",
      description: "Tatsächliche Notizen nach dokumentierten Kategorien.",
    ...
    },
    {
      ...
      description: "Chronologischer Überblick der veröffentlichten Produktnotizen.",
    ...
    },
    {
      ...
      description: "Echte Anwendungen aus KI, Automatisierung und Webentwicklung.",
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-[1600px] mx-auto px-4 md:px-8 lg:px-12 pt-4 pb-20 flex-grow flex flex-col">
        ...
        <div className="mt-14 flex flex-col items-center justify-center py-10 border-t border-border/50">
          ...
          >
            Dieses Archiv dokumentiert technische Entscheidungen, Produktstände und Erkenntnisse.
            Jede Galerieansicht stammt aus einem echten Repository oder dokumentierten Abnahmestand. © 2026 Leon Pösken.
          </motion.p>
        </div>
      </div>
