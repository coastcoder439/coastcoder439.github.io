...
const showcaseMembers = [
    ...portfolioData.projects.filter(project => Boolean(project.image)).slice(0, 5).map(project => ({
        id: project.id,
        name: project.title,
        role: project.role || 'Produktentwicklung',
        description: project.description,
        period: project.status === 'ongoing' ? 'In Entwicklung' : 'Abgeschlossen',
        image: project.image!,
        social: { website: project.demoUrl || project.repoUrl || ('/projects/' + project.slug) }
    })),
    {
        id: 'view-more',
        name: 'Alle Projekte',
        role: 'Acht echte Produkte entdecken',
        description: 'Alle Projekte mit belegten Screenshots, Live-Links und Repository-Material.',
        period: 'Portfolio',
...
const GALLERY_IMAGES = [
    ...
    "/projects/keel-harness/dashboard.png",
    "/projects/flowvoice/wave.png",
                ...
                <div className="relative z-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-12 border-t border-zinc-100 dark:border-zinc-900 pt-8 md:pt-12">
                        ...
                        <div className="md:col-span-7 flex flex-col sm:flex-row gap-8 text-[13px]">
                            ...
                            <span className="text-zinc-800 dark:text-zinc-200 font-bold uppercase tracking-widest block border-b border-zinc-100 dark:border-zinc-900 pb-3">Umfang & Plattform</span>
                            ...
                            <span className="text-zinc-800 dark:text-zinc-200 font-bold uppercase tracking-widest block border-b border-zinc-100 dark:border-zinc-900 pb-3">Anbindung</span>
                                <p className="text-zinc-500 leading-relaxed">
                        ...
                        >
                            ...
                            <div className="w-full pb-0">
                                <HorizontalTimeline data={showcaseMembers.map((member) => ({
                                    title: member.id === 'view-more' ? 'Alle Projekte entdecken' : (member.role || member.name),
                                    isEnd: member.id === 'view-more',
                                    period: 'period' in member ? member.period : undefined,
                                    content: member.id === 'view-more' ? (
                                        <Link
                                            href={member.social?.website || '/projects'}
